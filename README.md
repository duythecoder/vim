...
# CP Scripts for Neovim
> :warning: You might need to enable **unrestricted** ability of running Powershell scripts as listed [here](https://windowstect.com/set-powershell-execution-policy-as-unrestricted/) or some other related pages in order to run these scripts.

I created these scripts for ease in creating projects and debugging.
- [cp.ps1](https://github.com/duythecoder/nvim-settings/blob/main/ps-script/cp.ps1): Create a directory with a default C++ file.
  - Create a C++ default file, copy its path and replace `D:/CP/` at line 8.
  - Change `$dir`, default path `D:/CP/` to your desired project folder.
  - Run by CMD: `powershell "$path-to-cp.ps1" project-name cpp-name`, replace `$path-to-cp.ps1`, `project-name`, `cpp-name` with your `cp.ps1` location, your project name and your C++ file name.
